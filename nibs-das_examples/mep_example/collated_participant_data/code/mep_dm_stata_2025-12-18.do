/*******************************************************************************
Do-file title   NIBS-DAS_mep_dm_stata_2025-12-11.do    		 

Stata code file demonstrating principles of data management (DM) of a fictional MEP dataset: 
"NIBS-DAS_mep_exampledata_2025-11-11_copy.xlsx"
...
import excel using "usedata/NIBS-DAS_mep_exampledata_2025-12-13_copy.xlsx", first
...
* Check baseline (pre-TMS) APM EMG amplitude.
tab pre_rms_apb, missing
sum pre_rms_apb, detail

* Check APB muscle MEP amplitude by MEP type. 
...
/* Save cleaned uncollapsed dataset - a version with MEP amplitudes for each trial rather than collapsed by MEP block (below). 
Investigators may also wish to incorporate all MEPs in their analyses without collapsing by MEP 
block, e.g., by nesting MEPs within blocks of trials  */
save "usedata/NIBS-DAS_mep_exampledata_uncollapsed_wide.dta", replace 


********************************************************************************
* COLLAPSE DATA *
********************************************************************************
/* use collapse command to computes the mean of MEP amplitudes in FDI and APB muscles by participants and tms 
stimulation modes (dual or single pulses). These and similar operations should be done within software 
rather by editing data in spreadsheets before this step, to ensure all data are retained, and version/operation control.
...
* Save dataset
save "usedata/NIBS-DAS_mep_exampledata_meanblock_wide.dta", replace // Save mean MEP data in wide layout
...
need the data formatted in "long" format. This manipulation will produce a 
new 'Muscle' variable. */

*Print mean to check they match those after reshape
...
into a single variable named 'mep_ampl' (and does the same for pre_rms), which will be the DV below, and 
creates a new variable named 'muscle', to be used as an IV in the analyses. */
...
save "usedata/NIBS-DAS_mep_exampledata_meanblock_long.dta", replace 
